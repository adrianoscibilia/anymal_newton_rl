###########################################################################
# Example of Anymal Robot control via keyboard
#
...
class RobotConfig:
    ...
    asset_path: str  # USD asset path inside the downloaded asset directory (default)
    urdf_filename: str  # URDF filename inside the project's robot_description/ subdirectory (external_model)
...
ROBOT_CONFIG = RobotConfig(
    ...
    urdf_filename="anymal_c_simple_description/urdf/anymal.urdf",
...
class Example:
    def __init__(
        ...
        if urdf_path is None:
            # Default: load from downloaded newton-assets (USD)
            builder.add_usd(
            ...
            # External model: load from local robot_description/ URDF
            builder.add_urdf(
                urdf_path,
                xform=wp.transform(wp.vec3(0, 0, 0.8)),
                floating=True,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Load robot model from local robot_description/ URDF instead of downloaded newton-assets USD.",
    )
    ...
    # Determine URDF path: None = use default USD asset, path = use local URDF
    if args.external_model:
        urdf_path = str(Path(__file__).parent / "robot_description" / robot_config.urdf_filename)
        print(f"[INFO] Using external URDF model: {urdf_path}")
        if not Path(urdf_path).exists():
            print(f"[ERROR] URDF file not found: {urdf_path}")
            print(f"[INFO] Please place the URDF file at: {urdf_path}")
            exit(1)
    else:
        urdf_path = None
        print(f"[INFO] Using default USD model from assets: {asset_directory}/{robot_config.asset_path}")
    ...
    example = Example(viewer, robot_config, config, asset_directory, urdf_path, mjc_to_physx, physx_to_mjc)
